...
class AccountGoldSpec extends ObjectBehavior
{
    function it_is_initializable(\Client $client)
    {
        $this->beConstructedWith($client);
        $this->shouldHaveType(AccountGold::class);
    }
    ...
    public function it_credits_a_pyme_from_banelco(\Client $client)
    {
        ...
        $this->getBalance()->shouldBeLike(Money::USD(9997));
    }

    public function it_credits_a_pyme_from_link(\Client $client)
    {
        $client->beADoubleOf(\Pyme::class);
        $this->beConstructedWith($client);


        $this->credit('Banelco deposit', Money::USD(5000000), Date::now(), \Origin::link())->shouldReturnAnInstanceOf(\Transaction::class);
        $this->getTransactions()->shouldHaveCount(1);
        $this->getBalance()->shouldBeLike(Money::USD(5000000));
    }
